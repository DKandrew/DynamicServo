...
void setPosition(int fd, int id, int position){
	...
	int checksum = ~((id + len + AX_WRITE_DATA + AX_GOAL_POSITION_L + posi_l + posi_h) & 0xff);
	int dataLen = 9; 		// Data length is 9 = len + 4. 4 represnets FF FF id len
	int data[dataLen] = {0xFF, 0XFF, id, len, AX_WRITE_DATA, AX_GOAL_POSITION_L, posi_l, posi_h, checksum};
